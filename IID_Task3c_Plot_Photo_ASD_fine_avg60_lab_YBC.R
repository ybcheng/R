...
# example output: IID_Task3c_Plot_Photo_ASD_fine_avg60_YBC.pdf
...
#Photo_Folder = "K:/IID_SaltonSea/Tasks/PotentialPilotStudies/PoeRoad/PhotoDocumentation/Final"
...
csv_file = "K:/IID_SaltonSea/Tasks/Soil mapping/ASD/Processing/4PLS/SanFelipeWash_V2/SFW_C1_C2_S1_S2_wavg60_V2_lab.csv"
...
SavePDF = "K:/IID_SaltonSea/Tasks/Soil mapping/ASD/Processing/Charts/SFW_C1_C2_S1_S2_wavg60_V2_lab1.pdf"
...
for (jpgfile in FEIDjpg_Unique) {
  ...
  Lab_subset <- data.frame(x=iData2_subset2$Z, fine=iData2_subset2$lab_fine)
  Lab_subset <- na.omit(Lab_subset)
  ...
  legend('bottomright', c("Lab Fine"), inset = c(0.6,0), horiz = TRUE, bty='n', pch=19, cex=0.4, col=c("cornflowerblue"))
  #legend('bottomright', c("Sand (AVERAGE)","Fine (AVERAGE)", "Lab Fine"), inset = c(0,0), horiz = TRUE, lty=c(3,3), lwd=2,  bty='n', cex=0.4, col=c("Red","Green", "cornflowerblue"))
  ...
  if (length(Lab_subset$fine) > 0){
    points(Lab_subset$fine, Lab_subset$x, col="cornflowerblue", lwd=2, pch=19, cex=0.75)
  }
